...
class NYSECalendar:
    ...
    def get_next_trading_date(cls, from_date=None):
        ...
        next_date = from_date + __import__('datetime').timedelta(days=1)
        
        while True:
            if next_date.weekday() >= 5:
                next_date += __import__('datetime').timedelta(days=1)
                continue
...
def main():
    ...
    try:
        ...
        # Upload to HF Hub (if uploader available)
        try:
            ...
            print("HF uploader not available, skipping upload")
