...
def load_latest_signals_from_hf() -> Optional[Dict]:
    try:
        ...
        path = hf_hub_download(repo_id=HF_RESULTS_REPO, filename=latest_file, repo_type="dataset")
        with open(path) as f:
            signals = json.load(f)
...
def render_header():
    ...
    with col2:
        ...
        """, unsafe_allow_html=True)
    st.markdown('<div class="warning-banner">⚠️ Research Only · Not Financial Advice · Targets maximum predicted return with predictability filtering · Signals valid for next NYSE trading session only</div>', unsafe_allow_html=True)
...
def render_hero_top3(signals: Dict):
    ...
    <div style="text-align: center; margin-bottom: 1rem;">
        <div style="font-size: 0.9rem; color: #666; text-transform: uppercase; letter-spacing: 2px;">Target: Maximum Predicted Return</div>
    ...
    with cols[0]:
        ...
        <div class="hero-card-main">
            ...
            <div class="hero-return" style="color: {primary_color};">{primary['predicted_1d_return_bps']:+.0f} bps <span style="font-size: 0.6em;">({primary['predicted_1d_return_pct']:+.3f}%)</span></div>
            <div style="margin-top: 0.5rem;">Predictability: {primary['predictability_index']:.2f} · {primary['regime'].upper()}</div>
        </div>
        """, unsafe_allow_html=True)
    if num_runners >= 1:
        r = runners[0]
        ...
        with cols[1]:
            st.markdown(f"""
            <div class="hero-card"><div class="hero-label">🥈 Rank #2</div><div class="hero-etf-secondary">{r['etf']}</div>
            <div class="hero-return" style="font-size: 1.2rem; color: {runner_color};">{r['predicted_1d_return_bps']:+.0f} bps</div>
            <div>{r['regime']} · p={r['predictability_index']:.2f}</div></div>
            """, unsafe_allow_html=True)
    if num_runners >= 2:
        r = runners[1]
        runner_color = '#90EE90' if r['predicted_1d_return_bps'] > 0 else '#FFB6C1'
        with cols[2]:
            st.markdown(f"""
            <div class="hero-card"><div class="hero-label">🥉 Rank #3</div><div class="hero-etf-secondary">{r['etf']}</div>
            <div class="hero-return" style="font-size: 1.2rem; color: {runner_color};">{r['predicted_1d_return_bps']:+.0f} bps</div>
            <div>{r['regime']} · p={r['predictability_index']:.2f}</div></div>
            """, unsafe_allow_html=True)
    st.markdown("""
...
def render_full_ranking(signals: Dict):
    ...
    # Ensure the column for predicted return exists (fallback to bps version)
    if 'predicted_1d_return' not in df.columns:
        if 'predicted_1d_return_bps' in df.columns:
            df['predicted_1d_return'] = df['predicted_1d_return_bps']
        else:
            df['predicted_1d_return'] = 0.0
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "predictability_index": st.column_config.ProgressColumn("Predictability", min_value=0, max_value=1, width="medium"),
    ...
    if 'predicted_1d_return' in df.columns:
        fig = px.histogram(
            df, x='predicted_1d_return', color='koopman_regime' if 'koopman_regime' in df.columns else None,
            nbins=20, title="Distribution of Predicted Returns Across Universe",
...
def render_koopman_analysis(signals: Dict):
    ...
    fig.add_trace(go.Scatter(x=np.cos(theta), y=np.sin(theta), mode='lines', line=dict(color='gray', dash='dash', width=2), name='Unit Circle'))
    colors = ['#dc3545' if r > 1.0 else '#ffc107' if r > 0.95 else '#28a745' for r in radii]
    fig.add_trace(go.Scatter(x=real, y=imag, mode='markers', marker=dict(size=8, color=colors, line=dict(width=1, color='black')), name='Koopman Modes'))
    fig.update_layout(title=f"Eigenvalue Spectrum for {primary['etf']} ({primary['regime']} regime)", xaxis_title="Real", yaxis_title="Imaginary", height=600, yaxis_scaleanchor="x", yaxis_scaleratio=1)
    fig.add_annotation(x=1.1, y=0, text="Expanding", showarrow=False, font=dict(color="#dc3545"))
    fig.add_annotation(x=0.7, y=0.7, text="Oscillatory", showarrow=False, font=dict(color="#ffc107"))
    fig.add_annotation(x=0.3, y=0, text="Contracting", showarrow=False, font=dict(color="#28a745"))
    st.plotly_chart(fig, use_container_width=True)
    st.caption("**Red**: Growth modes (|λ|>1) · **Yellow**: Oscillatory (|λ|≈1) · **Green**: Decay modes (|λ|<1)")
...
def render_sidebar():
    with st.sidebar:
        ...
        <div style="font-size: 0.8rem; color: #666;">
            <b>P2 Engine Suite</b><br>Koopman-Spectral v1.0.0<br>Research Only · Apr 2026
        </div>
        """, unsafe_allow_html=True)
...
def render_footer():
    st.markdown("""
    <div class="footer">
        <b>P2 Koopman-Spectral Engine v1.0.0</b> · Maximum Return Targeting Variant · NYSE Next-Open Scheduling<br><br>
        <b>⚠️ Research Disclaimer</b><br>This engine targets maximum predicted return using Koopman operator theory. 
        Signals are generated pre-market for the next NYSE trading session. Past performance does not guarantee future results. 
        All outputs are research-grade and do not constitute investment advice.<br><br>
        <b>Data Attribution</b><br>Input data: <code>P2SAMAPA/p2-etf-deepm-data/data/master.parquet</code> · 
...
def main():
    ...
    st.caption(f"Source: {signals.get('_source', 'Unknown')} | Loaded: {signals.get('_loaded_at', 'Unknown')}")
